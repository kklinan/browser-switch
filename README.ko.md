...
| 📋 **7가지 규칙 모드** | 도메인 일치 / 전체 URL 일치 / 와일드카드 / 정규식 / 포함 / 접두 / 접미. 우선순위 내림차순으로 평가 |
| 🖱️ **카드형 선택기** | 브라우저 아이콘 그리드. 4개를 넘으면 "더보기(⌘R)" 카드로 접힘 |
| ⌨️ **키보드 우선** | `⌘1`~`⌘9` 또는 숫자 키로 N번째 브라우저 실행. `⌘R`은 전체 목록 펼치기. `Enter`는 기본, `Esc`는 취소 |
| 🪟 **새 창 규칙** | 규칙마다 기존 창을 재사용하지 않고 항상 새 창에서 열도록 지정 가능 |
| 📝 **규칙 편집** | 규칙은 추가·삭제뿐 아니라 바로 수정도 가능 |
...
| `⌘R` | 전체 브라우저 목록 펼치기 — "더보기" 카드 클릭과 동일 |
| `Enter` | 기본 브라우저로 열기 |
| `Esc` | 아무것도 열지 않고 취소 |
| 상단 URL 바 클릭 | 전체 URL을 클립보드에 복사 |
| "이 도메인 기억" | 이번 선택을 `exact` 규칙으로 저장 |
| 톱니바퀴 / 복사 버튼 | 설정 열기 / URL을 클립보드에 복사 |

카운트다운이 0이 되면 강조된 카드가 아니라 **기본 브라우저**(설정의 `default_browser`)가 사용됩니다. 톱니바퀴 버튼으로 설정을 열면 **카운트다운이 일시 정지**되므로, 규칙을 편집하는 동안 선택기가 자동으로 열리지 않습니다.
...
- **규칙**——우선순위 내림차순으로 전체 규칙 표시. 추가·편집(✎)·삭제. 각 규칙은 **새 창에서 열기**로 지정 가능
...
| `urlequal` | URL 전체 완전 일치 | `https://github.com/a/b` → 해당 URL만, `?x=1`은 제외 |
| `wildcard` | host **또는** 전체 URL, `*` `?` 지원 | `*.google.com` → mail.google.com; `*/settings` → 모든 설정 페이지 |
| `regex` | host **또는** 전체 URL | `.*\.(test\|staging)\..*` |
| `contains` | host **또는** 전체 URL 부분 문자열 | `login` → example.com/login |
| `prefix` | host **또는** 전체 URL 접두 | `dev.` → dev.example.com; `https://` → 모든 보안 링크 |
| `suffix` | host **또는** 전체 URL 접미 | `.cn` → example.cn; `.pdf` → 모든 PDF 링크 |

- 규칙은 `priority` **내림차순**으로 평가되며 처음 일치한 것이 채택됩니다.
- 매칭 시 원본 host와 `www.`를 제거한 host를 모두 시도하므로, `example.com` 규칙은 `www.example.com`에도 일치합니다.
- `contains` / `prefix` / `suffix`는 와일드카드의 **간편 모드**(`*p*` / `p*` / `*p` 와 동등)입니다. glob이나 정규식을 쓰기 부담스러운 사용자를 위한 것이며, `*`와 `?`는 **해석되지 않고** 일반 문자로 취급됩니다. 그런 규칙은 절대 일치하지 않으므로 저장 시 차단됩니다.
- `exact` / `urlequal`을 제외한 모든 모드는 host와 전체 URL을 **모두** 시도하므로, 경로(`*/settings`)나 스키마(`https://`)에만 나타나는 내용도 정상적으로 일치합니다.
- "선택 기억"으로 생성된 규칙의 우선순위는 `100` 고정, 수동 추가는 기본 `50`입니다.
- 모든 규칙은 **새 창에서 열기**로 지정할 수 있습니다. Chrome / Edge / Safari는 단일 인스턴스 앱이라 `open -n`이 무시되므로, 대신 `--new-window`(Chromium/Firefox) 또는 AppleScript(Safari)를 사용합니다.

...
| 📋 **6 つのルールモード** | 完全一致 / ワイルドカード / 正規表現 / 部分一致 / 前方一致 / 後方一致。優先度の降順で評価 |
| 🖱️ **カード型ピッカー** | ブラウザアイコンのグリッド。4 個を超えると「その他」カードに折りたたみ |
| ⌨️ **キーボード優先** | `⌘1`〜`⌘9` または数字キーで N 番目のブラウザを起動。`Enter` でデフォルト、`Esc` でキャンセル |
...
カウントダウンが 0 になると、ハイライト中のカードではなく**デフォルトブラウザ**（設定の `default_browser`）が使われます。
...
| `wildcard` | host、`*` `?` 対応 | `*.google.com` → mail.google.com |
| `regex` | host **または**完全 URL | `.*\.(test\|staging)\..*` |
| `contains` | host **または**完全 URL の部分文字列 | `login` → example.com/login |
| `prefix` | host の前方 | `dev.` → dev.example.com |
| `suffix` | host の後方 | `.cn` → example.cn |
